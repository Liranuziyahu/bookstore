...
namespace BookstoreAPI.Controllers
{
    ...
    public class BooksController : ControllerBase
    {
        ...
        public IActionResult GetBooksReport()
        {
            ...
            sb.Append("<html><body><h1>Books Report</h1><table border='1'><tr><th>ISBN</th><th>Title</th><th>Authors</th><th>Category</th><th>Year</th><th>Price</th></tr>");

            foreach (var book in books)
            {
                sb.Append("<tr>");
                sb.AppendFormat("<td>{0}</td>", book.ISBN);
                sb.AppendFormat("<td>{0}</td>", book.Title);
                sb.AppendFormat("<td>{0}</td>", string.Join(", ", book.Authors));
                sb.AppendFormat("<td>{0}</td>", book.Category);
                sb.AppendFormat("<td>{0}</td>", book.Year);
                sb.AppendFormat("<td>{0}</td>", book.Price);
                sb.Append("</tr>");
            }
